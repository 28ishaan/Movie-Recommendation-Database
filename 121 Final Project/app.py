...
def rate_movie(conn, user_id):
    """
    Allows a user to rate a movie by title, updating the watch_history table.
    """
    ...
    try:
        # First, retrieve the movie_id based on the movie title
        cursor.execute("SELECT movie_id FROM movies WHERE movie_title = %s", (movie_title,))
        result = cursor.fetchone()
        if result:
            movie_id = result['movie_id']
            # Then, insert or update the rating in the watch_history table
            cursor.execute("INSERT INTO watch_history (user_id, movie_id, watch_date, user_rating) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE user_rating = %s", 
                           (user_id, movie_id, watch_date, rating, rating))
            conn.commit()
            print("Your rating has been recorded.")
        else:
            print("Movie title not found. Please check the title and try again.")
    except mysql.connector.Error as err:
