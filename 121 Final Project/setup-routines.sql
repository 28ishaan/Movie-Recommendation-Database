...
CREATE FUNCTION CalculateMovieRatingScore(in_movie_id INT)
...
BEGIN
    ...
    SELECT COALESCE(AVG(user_rating), 0) INTO avg_user_rating FROM watch_history WHERE movie_id = in_movie_id;
    SELECT COALESCE(imdb_rating, 0) INTO imdb_rating FROM ratings WHERE movie_id = in_movie_id;
...
CREATE PROCEDURE UpdateMovieRating(in_movie_id INT)
BEGIN
    DECLARE new_rating DECIMAL(3, 2);
    SET new_rating = CalculateMovieRatingScore(in_movie_id);
    UPDATE ratings SET imdb_rating = new_rating WHERE movie_id = in_movie_id;
END;
